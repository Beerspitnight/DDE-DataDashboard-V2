...
############################################
# You had "uploaded_file = st.sidebar.file_uploader(...)" repeated twice.
...
if uploaded_file:
    ...
    # (B) Check required columns  # <-- MOVED HERE
    ...
    if iready_vis_type == "Scatter Plot":
        col_x = st.sidebar.selectbox(
            ...
            index=1,  # Default 'Lessons Passed'
            key="iready_x"
        )
        col_y = st.sidebar.selectbox(
            "Y-axis", 
            options=iready_columns, 
            index=3,  # Default '% Lessons Passed'
            key="iready_y"
        )
    ...
    # Identify the three columns: Passed, Completed, and Percentage
    ...
    # Sort by the "Passed" metric (optional)
    class_avg = class_avg.sort_values(passed_col, ascending=False)

    # Create a subplot figure with a secondary y-axis
    fig = make_subplots(specs=[[{"secondary_y": True}]])
        
    # Add 'Lessons Passed' bar trace (primary y-axis)
    fig.add_trace(
        go.Bar(
    ...
    # Add 'Lessons Completed' bar trace (primary y-axis)
    fig.add_trace(
        go.Bar(
            x=class_avg["Class Teacher(s)"],
    ...
    # Add '% Lessons Passed' bar trace (secondary y-axis)
    fig.add_trace(
        go.Bar(
            x=class_avg["Class Teacher(s)"],
            y=class_avg[pct_col],
    ...
    **Heatmap Explanation:**
    ...
    ###########################################################
    #  THIS 'ELSE' RUNS WHEN NO FILE HAS BEEN UPLOADED.      #
    #  We DON'T reference `data` here, because it doesn't    #
    #  exist until a file is read.                           #
    ###########################################################
